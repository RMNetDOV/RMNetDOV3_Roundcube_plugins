<?php
class rmnetdov3_pass extends rcube_plugin
{
    ...
    function init()
    {
        ...
        $this->register_action('plugin.rmnetdov3_pass', [$this, 'init_html']);
        $this->register_action('plugin.rmnetdov3_pass.save', [$this, 'save']);

        if (strpos($this->rcmail->action, 'plugin.rmnetdov3_pass') === 0) {
    ...
    function init_html()
    {
        ...
        $this->rcmail->output->send('rmnetdov3_pass.pass');
    }
            ...
            if ($confirm && $this->rcmail->decrypt($_SESSION['password']) != $curpwd) {
                ...
                if (!$error) {
                    try {
                        ...
                        $rmnetdov_version = $soap->server_get_app_version($session_id);
    ...
    function gen_form($attrib)
    {
        $this->rcmail->output->add_label(
            'rmnetdov3_pass.nopassword',
            'rmnetdov3_pass.nocurpassword',
            'rmnetdov3_pass.passwordinconsistency',
            'rmnetdov3_pass.changepasswd',
            'rmnetdov3_pass.passwordminlength'
        );
        ...
        $out = $this->rcmail->output->request_form([
                ...
                'method'  => 'post',
                'task'    => 'settings',
                'action'  => 'plugin.rmnetdov3_pass.save',
                'noclose' => true
            ] + $attrib);
