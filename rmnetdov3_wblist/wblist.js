window.rcmail && rcmail.addEventListener('init', function (evt) {
    rcmail.register_command('plugin.rmnetdov3_wblist.save', function () {
        var input_email = rcube_find_object('_wblistemail');

        if (input_email.value == '') {
            parent.rcmail.display_message(rcmail.gettext('rmnetdov3_wblist.textempty'), 'error');
        }
        // Test for user@example.com or @example.com syntax
            ...
            parent.rcmail.display_message(rcmail.gettext('rmnetdov3_wblist.invalidaddress'), 'error');
            input_email.focus();
        }
        else {
    ...
    rcmail.register_command('plugin.rmnetdov3_wblist.del', function (id, type) {
        rcmail.confirm_dialog(rcmail.gettext('rmnetdov3_wblist.wblistdelconfirm'), 'delete', function(e, ref) {
            var lock = ref.set_busy(true, 'loading');
            ref.http_request('plugin.rmnetdov3_wblist.del', '_id=' + id + '&_type=' + type, lock);
    ...
    if (rcmail.env.action.startsWith('plugin.rmnetdov3_wblist')) {
...
function wb_edit(id, type) {
    rcmail.location_href(rcmail.env.comm_path + '&_action=plugin.rmnetdov3_wblist&_id=' + id + '&_type=' + type, window, true);
}
