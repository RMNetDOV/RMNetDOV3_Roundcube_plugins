...
class rmnetdov3_account extends rcube_plugin
{
    ...
    const RMNETDOV_PLUGINS = [ 'account', 'pass', 'fetchmail', 'forward', 'autoreply', 'filter', 'wblist', 'spam' ];
    ...
    function init()
    {
        ...
        $this->register_action('plugin.rmnetdov3_account', [$this, 'init_html']);
        $this->register_action('plugin.rmnetdov3_account.show', [$this, 'init_html']);
        ...
        if (strpos($this->rcmail->action, 'plugin.rmnetdov3_account') === 0 ||
      ...
      *
      * Adds rmnetdov3_account settings section into preferences.
      */
    function settings_actions($args)
    {
        $args['actions'][] = [
            'action' => 'plugin.rmnetdov3_account',
            'class'  => 'account',
            'label'  => 'acc_acc',
            'title'  => 'acc_acc',
            'domain' => 'rmnetdov3_account',
        ];
    ...
    function accountlist($attrib)
    {
        ...
        $sections = [];
        foreach ($this::RMNETDOV_PLUGINS as $plugin) {
            if (isset($rc_plugins['rmnetdov3_' . $plugin])) {
                if ($plugin == 'account') {
                    $plugin = 'general';
                }
