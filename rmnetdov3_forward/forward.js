window.rcmail && rcmail.addEventListener('init', function (evt) {
    rcmail.register_command('plugin.rmnetdov3_forward.save', function () {
        var input_address = rcube_find_object('_forwardingaddress');

        if (input_address.value == '') {
            parent.rcmail.display_message(rcmail.gettext('rmnetdov3_forward.forwardingempty'), 'error');
        }
        else if (!rcube_check_email(input_address.value, false)) {
            parent.rcmail.display_message(rcmail.gettext('rmnetdov3_forward.invalidaddress'), 'error');
            input_address.focus();
        }
        else {
    ...
    rcmail.register_command('plugin.rmnetdov3_forward.del', function (email) {
        rcmail.confirm_dialog(rcmail.gettext('rmnetdov3_forward.forwarddelconfirm'), 'delete', function(e, ref) {
            var lock = ref.set_busy(true, 'loading');
            ref.http_request('plugin.rmnetdov3_forward.save', '_type=del&_forwardingaddress=' + email, lock);
    ...
    if (rcmail.env.action.startsWith('plugin.rmnetdov3_forward')) {
